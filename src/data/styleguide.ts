			...
			rules: [
				{
					...
					reviews: {
						...
						fable: {
							...
							rationale: "Domain words beat generic ones when a more specific word exists, and the example shows the gain. The claim that item, data and handle 'say nothing' is too absolute: in a generic collection helper, item is the honest name. I would revise the rationale to 'use the most specific word the context allows' before adopting it.",
							reviewedAt: "2026-09-17",
							reviewedHash: "fce4c577",
						},
					},
  ...
  await db.orders.update(orderId, { total: discountedTotal(order.total) });
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "somewhat-agree",
							rationale: "Callers should not lose data they still hold, and returning a new value is the right default. Performance is not the only reason for an explicit in-place API, builders and buffers are designed around it; what survives is the instruction to make mutation visible in the name or the type.",
							reviewedAt: "2026-09-17",
							reviewedHash: "81d5ac25",
						},
					},
  ...
  return await api.get("/invoices"); // let the caller show an error state
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "somewhat-disagree",
							rationale: "What happened, which value and what to do next is the right shape. As written, 'with which value' invites a token or a password into a message that ends up in a log. I would add one sentence before adopting it: never include a secret or personal data; redact by default.",
							reviewedAt: "2026-09-17",
							reviewedHash: "827c20b8",
						},
					},
...
const response = await fetch(url, { signal: AbortSignal.timeout(REPORT_TIMEOUT_MS) });`,
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "somewhat-disagree",
							rationale: "The policy is right: bounded, jittered retries of retryable errors at one layer, and the warning that nested retries multiply calls. The prefer example contradicts the rationale, because it bounds the attempts but not the total time. I would revise the example to check a deadline before each attempt.",
							reviewedAt: "2026-09-17",
							reviewedHash: "aeb97ff6",
						},
					},
...
retries += 1;`,
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "somewhat-disagree",
							rationale: "A TODO needs something that makes it actionable, and an issue link is the reliable form. 'Or do not write it' also deletes a local note with a concrete trigger such as 'remove after the v3 migration'. I would revise the rule to require an issue or a stated trigger.",
							reviewedAt: "2026-09-17",
							reviewedHash: "76f1daf3",
						},
					},
			...
			rules: [
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "neutral",
							rationale: "Keeping a component next to its test and styles is good practice. Whether to organize by feature or by file type depends on the size of the codebase, the toolchain and the ownership boundaries, and I have no general preference to record. Context decides.",
							reviewedAt: "2026-09-17",
							reviewedHash: "3e64f9cf",
						},
					},
...
deploys. One retry with a 2 s delay covers it, so we stop paging on-call.`,
				...
				},
				{
					...
					reviews: {
						...
						fable: {
							rating: "somewhat-disagree",
							rationale: "Secrets: never, no exceptions. Generated files: build output, yes, but a literal reader also ignores the lockfile and the generated types that reviewers must see. I would split the rule, because one half is a security rule and the other a hygiene default with exceptions.",
							reviewedAt: "2026-09-17",
							reviewedHash: "b6b21bf9",
						},
					},
